        ...
        <div className="space-y-8 font-outfit pb-16">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                {[
                    ...
                    <div key={card.label} className={`bg-white rounded-[2rem] p-6 border shadow-sm relative overflow-hidden ${card.alert ? 'border-rose-200 ring-4 ring-rose-50' : 'border-slate-50'}`}>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-50 shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-slate-50">
                  ...
                  ${shop.is_active === false
                    ...
                    {topShops.length === 0 && (
                        <div className="py-20 text-center text-[10px] font-black text-slate-300 uppercase tracking-widest">No shops registered yet</div>
                    )}
                </div>
            </div>
                ...
                <div className="space-y-3">
                    {recentActivity.map((order) => (
                        <div key={order.id} className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all">
                            <div className="w-8 h-8 rounded-xl bg-blue-600/20 flex items-center justify-center text-sm">🖨️</div>
                            <div className="flex-1 min-w-0">
                                <p className="text-xs font-black text-white truncate">{order.print_shops?.name || 'Unknown Shop'} — #{order.id?.slice(-6)}</p>
                                <p className="text-[9px] font-black text-slate-500">{new Date(order.created_at).toLocaleString('en-IN', { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'short' })}</p>
                            </div>
                            <span className={`px-3 py-1 rounded-xl text-[8px] font-black uppercase tracking-widest ${order.status === 'completed' ? 'bg-emerald-500/20 text-emerald-400' : ['printing', 'shop_accepted'].includes(order.status) ? 'bg-blue-500/20 text-blue-400' : 'bg-amber-500/20 text-amber-400'}`}>
