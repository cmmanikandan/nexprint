...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        const checkUser = async () => {
            ...
            if (profile?.role === 'shop_owner' || profile?.role === 'staff') {
                // Shop owner accidentally on admin path → redirect to their dashboard
        ...
        <div className="min-h-screen bg-slate-50 font-outfit text-slate-900 select-none">
            ...
            <aside className="fixed left-0 top-0 w-64 h-full bg-slate-900 text-white z-50 hidden lg:flex flex-col">
                ...
                <div className="px-4 pb-4">
                    <Link
                        href="/dashboard"
                        className="flex items-center gap-3 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest text-blue-400 hover:bg-blue-400/10 transition-all border border-blue-400/20 mb-2"
                    >
                        <Store size={16} />
                        Switch → Shop View
                    </Link>
                    ...
                    <div className="flex items-center gap-4 lg:gap-8">
                        ...
                        <div className="flex items-center gap-3 pl-6 border-l border-slate-100">
                            <div className="hidden sm:block text-right">
                                <p className="text-[11px] font-black text-slate-900 leading-none">Super Admin</p>
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-widest transition-all duration-300 absolute bottom-3 ${active ? 'opacity-100' : 'opacity-0'}`}>
                                {item.label}
                            </span>
                            {active && <div className="absolute -top-1 w-8 h-1 bg-blue-500 rounded-full" />}
                        </Link>
                    );
                })}
            </nav>
